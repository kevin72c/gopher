...
func GetHtml(Insurl string) (html string) {
	// 解析代理地址
	proxy, err := url.Parse("http://127.0.0.1:10809") //加载本地代理
	//设置网络传输
...
func main() {
	homepage := "https://instagram.com/xx/"
	//fmt.Scanln(&homepage)
	queryHash := "44efc15d3c13342d02df0b5a9fa3d33f"
	first := 12
	//通过html获取第一条json地址
	...
	//id := GetIdByHtml(html)
	id := "30543244020"
	firstQueryUrl := SetQueryUrl(queryHash, id, strconv.Itoa(first), after) //  第一条json地址

	//通过json获取内容
	jsonContent := GetHtml(firstQueryUrl)
	for {
		ins := Json2Struct(jsonContent)
		nextAfter := GetAfter(ins) //通过json获得after的值
		nextQueryUrl := SetQueryUrl(queryHash, id, strconv.Itoa(first), nextAfter)
		GetDownloadUrl(GetUserName(homepage), ins)
		jsonContent = GetHtml(nextQueryUrl)
